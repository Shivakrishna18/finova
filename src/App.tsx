...
function LandingPage() {
  ...
  useEffect(() => {
    ...
    const onCtaClick = (event: MouseEvent) => {
      ...
      if (label.includes('Start Your Financial Journey') || label.includes('Start With FINOVA') || label.includes('Get Started')) window.location.href = '/app'
    }
  ...
  return <main ref={shellRef} className="site-shell">
    {!isTouch && <MagicCursor fillColor="#86eaff" cursorSize={27} enableGlow glowColor="#4cafff" glowIntensity={35} label={false} />}
    <div className="grain" aria-hidden="true" />
    <header className="nav-wrap"><a className="wordmark" href="#overview">FINOVA<span>.</span></a><button className="mobile-menu" aria-label="Toggle navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? '×' : '☰'}</button><nav className={menuOpen ? 'nav-links open' : 'nav-links'}><a href="#overview">Overview</a><a href="#how-it-works">How it Works</a><a href="#intelligence">Intelligence</a><a href="#features">Features</a><a href="#security">Security</a></nav><div className="nav-actions"><button onClick={() => go('final-cta')}>Sign In</button><button className="nav-cta" onClick={() => go('final-cta')}>Get Started <span>↗</span></button></div></header>

    <section id="overview" className="hero-section"><div className="hero-atmosphere hero-atmosphere-left" aria-hidden="true"><InteractiveHeroCanvas backgroundColor="rgba(5, 6, 10, 0)" lineColor="rgba(78, 150, 255, .2)" lineCount={42} speed={2.5} glow={18} /></div><div className="hero-atmosphere hero-atmosphere-right" aria-hidden="true"><ShaderGroupSwitcher background="#05060a" tint="#5277ff" brightness={32} speed={24} hover={30} style={{ minWidth: 0, minHeight: 0 }} /></div><div className="hero-copy"><p className="eyebrow">FINOVA <span>/// AI PERSONAL FINANCIAL OPERATING SYSTEM</span></p><h1>Your Personal<br /><em>Financial OS.</em></h1><p>FINOVA doesn't just track your money. It understands your financial life, predicts what happens next, protects your goals and helps you make smarter decisions.</p><div className="hero-actions"><LiquidCarveButton label="Start Your Financial Journey" colors={{ fill: '#e9f4ff', textColor: '#061018' }} blob={{ color: '#52d8ff', size: 120, smoothness: 60 }} padding="17px 23px" rounded={50} font={{ fontFamily: 'Manrope', fontWeight: 700, fontSize: 12 }} /><button className="text-action" onClick={() => go('how-it-works')}>Explore FINOVA <span>↓</span></button></div></div><div className="core-stage"><div className="core-label"><i /> FINOVA INTELLIGENCE CORE</div><div className="core-visual"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><PlasmaRing background="rgba(0,0,0,0)" colors={['#64eaff', '#5c76ff', '#bb54ff']} density={82} speed={44} waveHeight={12} centerOpacity={16} scale={31} style={{ width: '100%', height: '100%' }} /></div><FloatingSignal className="signal-balance" title="CURRENT BALANCE" value="₹1,24,850" /><FloatingSignal className="signal-spend" title="MONTHLY SPEND" value="₹38,420" /><FloatingSignal className="signal-goal" title="GOAL PROGRESS" value="72%" /><FloatingSignal className="signal-safe" title="SAFE TO SPEND" value="₹14,800" /></div><div className="scroll-cue">Scroll to enter the system <i /></div></section>

    <section id="how-it-works" className="problem-section content-width"><p className="section-index">01 / THE PROBLEM</p><LineMaskSplit text="Your money is more than a number." tag="h2" splitMode="lines" font={{ fontFamily: 'Manrope', fontSize: 'clamp(2.6rem, 6vw, 6rem)', fontWeight: 400, lineHeight: 1, textAlign: 'left' }} color="#eef3ff" /><p className="lead-copy">Traditional finance apps show you what happened.<br /><em>FINOVA helps you understand what happens next.</em></p><div className="concept-grid"><Concept number="01" title="TRACK" text="Understand where your money goes." /><Concept number="02" title="UNDERSTAND" text="Discover your financial behavior." /><Concept number="03" title="PREDICT" text="See the consequences before you spend." /></div></section>

    <section id="intelligence" className="brain-section content-width"><div className="section-heading"><div><p className="section-index">02 / THE FINANCIAL BRAIN</p><h2>One intelligence.<br /><em>Your entire financial life.</em></h2></div><p>FINOVA continuously brings your financial signals together to build a living understanding of your financial situation.</p></div><div className="brain-visual"><div className="brain-atmosphere"><InteractiveHeroCanvas backgroundColor="rgba(0,0,0,0)" lineColor="rgba(103,221,255,.16)" lineCount={28} speed={1.5} glow={30} /></div><div className="brain-core"><PlasmaRing background="rgba(0,0,0,0)" colors={['#8ef2ff', '#637aff', '#bc5cff']} density={60} speed={32} centerOpacity={12} scale={30} style={{ width: '100%', height: '100%' }} /><span>FINOVA<br /><small>INTELLIGENCE</small></span></div>{brainNodes.map((node, index) => <span key={node} className={`brain-node brain-node-${index}`}>{node}</span>)}</div></section>
    ...
    <section id="final-cta" className="final-section content-width"><div className="final-core"><PlasmaRing background="rgba(0,0,0,0)" colors={['#8ef2ff', '#6076ff', '#c560ff']} density={70} speed={34} centerOpacity={10} scale={30} style={{ width: '100%', height: '100%' }} /></div><p className="section-index">12 / THE NEXT STEP</p><h2>Don't just manage money.<br /><em>Understand it.</em></h2><p>Build a financial system that thinks with you.</p><div className="hero-actions"><LiquidCarveButton label="Start With FINOVA" colors={{ fill: '#e9f4ff', textColor: '#061018' }} blob={{ color: '#52d8ff', size: 120 }} padding="17px 23px" rounded={50} font={{ fontFamily: 'Manrope', fontWeight: 700, fontSize: 12 }} /><button className="text-action" onClick={() => go('intelligence')}>Explore the Intelligence <span>↗</span></button></div></section>
...
export default function App() {
  return window.location.pathname.startsWith('/app') ? <AppExperience /> : <LandingPage />
}
